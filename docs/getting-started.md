...
Greetings, friend! In this tutorial, we are going to cover creating an API using
...
If you are not familiar with NestJS, it is a Node based framework heavily
...
For this tutorial, we are creating an mission board for a fictional agency known
...
To get started, you will need to have a relatively newer of Node installed and a
...
Nest application will serve the API on locally during development.
...
Now if you go back to Cypress, you'll see a warning that it can't verify the
server is running. This is just a little hint that we haven't started up our
server yet. Let's go ahead and do so:
...
By this time, you will probably need to open several command windows or tabs to
...
Hit the "Create new empty spec" button and name the spec **missions.cy.ts** and
...
`@Controller` decorator. The decorator adds some meta-data to the class that the
framework uses to determine which controller will response to which HTTP
request. In this case, the `MissionsController` will response to any requests
...
`MissionsController` to the controllers array.
...
Back in the test runner, and hit the Run all specs button to try the test again.
This time the test will pass:

![Run all tests](/img/run_all_tests.jpg)

On the right, we can see the results and browse through the returned response
(our hard coded missions array), the headers, and cookies.
...
Instead of using the hard coded array, we will create a service that will be
...
Like a controller, a service is a JavaScript class marked with a decorator.
...
`description` with a type string, a `complete` with a type of boolean, and a
`created` with a type of string that will contain a ISO date string of when the
mission was added.

Next, we define a default mission that we will use to seed the database with.
...
to contain the `defaultMission` when the services initializes. In a real app, we
would instead call into a data layer or ORM like [TypeORM](https://typeorm.io/)
or [Prisma](https://www.prisma.io/) to manage data.

The `getList()` method returns everything that is currently in the array.
...
Next, let's update the API to be able to retrieve a specific mission via it's
...
This test starts off much like the last one, except we specify we want to pull
...
that the body of the response includes the mission we expect.
...
> By default, calls to `cy.api()` will fail if they don't return back a success
...
One of which is the `NotFoundException`. We can throw this error in any place we
...
Nest handle the dirty work from there:
...
The only thing above we haven't yet seen above is passing the new mission as an
object to the third parameter of the `cy.api()` method. The api method will
...
use the push method to add the new mission to the list then return it.
...
code, which represents a new entity was created as a result of the request.
...
The first time we run these tests, they will pass. However, the next run we will
...
One method is to reset the database into a known state before each test is run.
This is often difficult to do, but because our database is so small, we have
that luxury. Let's see how we can accomplish that.
...
Above, we are just overrriding the array by setting it to an array that only
...
Make sure you restart the Nest server so the change can take affect.

A bit later, we will take a look at how to secure this endpoint even more so the
calling API will need a token in order to call it.

Back in the tests, we want to make sure that we reseed the database before each
...
Above, we use the `cy.request()` command, which is the built in command for
...
the previous test getting in the way.

## Protecting Routes

Above in the `reset()` method on the controller, we put one safe guard in place
...
A guard is another JavaScript class that has one required method, `canActivate`.
...
method, and then we check that an `authorization` header as the correct token.
We are using a simple string here, but this token could be more complex like a
JWT or API key that could be validated.

If either of the above checks don't pass, we return false, which instructs Nest
to end the request and send back a 403 Forbidden status code. If they are both
good, then we return true and the request will continue to the controller.
...
Visit the Nest docs on more ways to bind guards to your application.
...
We pretty much have a working API to manage our missions, however, we haven't
...
Our `Mission` model is fairly simple, but lets add some validation to it to
ensure that any time one is added or updated, that we make sure its good.
...
Currently our `Mission` model is an interface. In order to annotate it with
